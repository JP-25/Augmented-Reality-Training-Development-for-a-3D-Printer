...
# Video Capture
cap = cv2.VideoCapture('vid98.mp4')
...
while index<15:
    ...
    fl_old = focal_length
    focal_length = (area_choice[1][0] * KNOWN_DISTANCE) / KNOWN_WIDTH
    # ignores random spikes in value due to bad video quality
    if (focal_length < 2*fl_old) & (focal_length > 0.5*fl_old):
        print("fl: ", focal_length)
        FLTotal = FLTotal + focal_length
...
# gives us a average value for the focal length
FLavg = FLTotal/index
print("focal length avg:", FLavg)
